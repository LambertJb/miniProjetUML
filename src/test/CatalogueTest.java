...
public class CatalogueTest {
	...
	public void testGetNomProduits_unProduitAvecNomEspacesAuDebut() {
		String[] tab = { "Mars" };
		cat.addProduit(" Mars", 10, 1);
		assertArrayEquals("recupère nom produit avec espace debut ; les espaces au début ne doivent pas être stockés",
	...
	public void testGetNomProduits_unProduitAvecNomEspacesALaFin() {
		String[] tab = { "Mars" };
		cat.addProduit("Mars ", 10, 1);
		assertArrayEquals("recupère nom produit avec espace fin ; les espaces à la fin ne doivent pas être stockés",
				tab, cat.getNomProduits());
	}
	...
	public void testGetNomProduits_unProduitAvecNomTabulationsAuDebut() {
		...
		cat.addProduit("	Mars", 10, 1);
		assertArrayEquals(
				"recupère nom produit avec tabulation debut ; les tabulations au début ne doivent pas être stockés",
				tab, cat.getNomProduits());
	}
	...
	public void testGetNomProduits_unProduitAvecNomTabulationsALaFin() {
		...
		cat.addProduit("Mars	", 10, 1);
		assertArrayEquals(
				"recupère nom produit avec tabulation fin ; les tabulations à la fin ne doivent pas être stockés", tab,
				cat.getNomProduits());
	}
	...
	public void testGetNomProduits_unProduitAvecNomTabulationsAuMilieu() {
		...
		cat.addProduit("Kit	Kat", 10, 1);
		assertArrayEquals(
				"recupère nom produit avec des tabulations au milieu ; les tabulations au milieu doivent être remplacées par des espaces",
				tab, cat.getNomProduits());
	}
	...
	public void testGetNomProduits_plusieursProduitsInseresOrdreAlphabetique() {
		...
		cat.addProduit("Raider", 12, 2);
		cat.addProduit("Treets", 10, 1);
		assertArrayEquals("recupère nom de plusieurs produits ajoutés dans ordre alphabétique", tab,
				cat.getNomProduits());
	}
	...
	public void testGetNomProduits_plusieursProduitsInseresOrdreAleatoire() {
		...
		assertArrayEquals(
				"recupère nom de plusieurs produits ajoutés dans ordre aléatoire (doivent être retournés dans l'ordre alphabétique)",
				tab, cat.getNomProduits());
	}
	...
	public void testClear() {
		String resultatAttendu = "\n" +
				 "Montant total TTC du stock : 0,00 €";
		cat.addProduit("Mars", 15,6);
		cat.addProduit("Treets", 10, 5);
		cat.clear();
		assertEquals("on doit n\'avoir aucun produit dans la liste de produit",resultatAttendu, cat.toString());
	}
